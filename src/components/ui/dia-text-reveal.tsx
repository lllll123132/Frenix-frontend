...
export interface DiaTextRevealProps extends Omit<
  ...
  "ref" | "style" | "animate" | "transition" | "color"
> {
  /**
   * Text or elements to reveal.
   */
  children?: React.ReactNode
  /**
   * Text to reveal (legacy/array support). Pass multiple strings to rotate when {@link DiaTextRevealProps.repeat} is `true`.
   */
  text?: string | string[]
  /**
...
export function DiaTextReveal({
  ...
  useEffect(() => {
    if (!spanRef.current || !isMulti) return
  ...
  useEffect(() => {
    ...
    // Always start on view if requested, but for hero we usually want it immediately
  ...
  return (
    <motion.span
      ref={spanRef}
      className={cn("relative inline-block align-bottom leading-[1.2] text-inherit", className)}
      style={{
        color: "transparent",
        backgroundClip: "text",
        ...
        ...(isMulti && measuredWidths.length > 0 && {
          overflow: "hidden",
          whiteSpace: "nowrap",
          width: fixedWidth ? Math.max(...measuredWidths) : measuredWidths[activeIndex],
        }),
      }}
